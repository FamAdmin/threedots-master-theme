      ...
      $.each(cart.items, function(index, item) {
        ...
        if (item.image) {
          cart_items_html += '<div class="cart_image">' +
              '<img src="' + item.image.replace(/(\.[^.]*)$/, "_compact$1").replace('http:', '') + '" alt="' + htmlEncode(item.title.split('|')[0] + ' - ' + item.title.split('-')[1]) + '" />' +
            '</div>';
        }

        cart_items_html += '<div class="cart_item__title"><div class="item_title">' + item.title.split('|')[0] + ' - ' + item.title.split('-')[1];
