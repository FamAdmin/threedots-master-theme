  ...
  $(".quick-view").click(function () {
    ...
    jQuery.getJSON('/products/' + product_handle + '.js', function (product) {
      ...
      $(options).each(function (i, option) {
        var opt = option.name;
        $('.qv-product-options').append('<div class="option-selection-' + opt.toLowerCase() + '"><span class="option">' + opt + '</span><span class="option-' + i + ' option ' + opt.toLowerCase() + '"></span></div>');
        $(option.values).each(function (i, value) {
           ...
           for( i = 0; i <= variants.length ; i++) { 
              ...
              var obj= variants[i];
             console.log(obj);
             if((obj.featured_image == null) || (obj.featured_image == '')){
             	var obj_src     = '';
             }else{
               var obj_src     = obj.featured_image.src;
             }
             var obj_alt     ='';
              var obj_option1 = obj.option1;
              var obj_title   = obj.title;

            var image_embeds = '<div class="swatch_style" style="background-image:url(' + obj_src + ')"><p></p></div>';
          $('.option-0').append(image_embeds);
              console.log(obj_src);
           
             }
         $('.option.' + opt.toLowerCase()).append('<label for="' + value + '"><input type="radio" value="' + value + '">' + value + '</input></label>');          
      })
      ...
      var product_options = product.options;$(product.variants).each(function (i, v) {
        console.log('come');
...
//           $('select.option-2').val(v.option3);
          console.log(price + 'here i am')
          console.log(v.option2 + 'here i am')
