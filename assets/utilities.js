  ...
  linkGalleryAndCarousel: function($gallery, $carousel){
    ...
    $gallery.on( 'select.flickity', function() {
      ...
      if (galleryData){
        ...
        //Blocking Scrolling Issue affecting multiple viewports, ToDo Investigate what was this intended for
        /*setTimeout(function() {
          var selectedThumbnail = document.getElementsByClassName('is-nav-selected');
          selectedThumbnail[0].scrollIntoView({
            behavior: "smooth",
            block: "nearest",
            inline: "nearest"
          });
  ...
  if ($('.collection-matrix').length) {
    var infiniteScroll = new Waypoint.Infinite({
      ...
      onAfterPageLoad: function(data){
        // IMPORTANT: Dynamically triggers adding metafield swatches to quickview
        fetch("?page="+nextPage).then(response => response.text()).then(data => {
          var doc = $(data);
          qv_scripts = doc.find(".metafield-script");
          //collection_swatches_scripts = doc.find(".collection-swatches-scripts");
          qv_scripts.appendTo("html");
          //collection_swatches_scripts.appendTo("html");
        });
        nextPage += 1;
  ...
  if ($('.search-matrix').length) {
    var infiniteSearchScroll = new Waypoint.Infinite({
      ...
      onAfterPageLoad: function(data){
        ...
        fetch("?page="+nextPage).then(response => response.text()).then(data => {
          var doc = $(data);
          scripts = doc.find(".metafield-script");
          scripts.appendTo("html");
        });
        nextPage += 1;
...
var quickShop = {
    ...
    $('body').on('click', '.js-quick-shop-link', function(e){
      ...
      $.fancybox.open($('.js-quick-shop'), {
        ...
        afterLoad: function(){
		  ...
		  $('.js-product-form .swatch_options label[data-index="1"]').trigger('click');
  ...
  afterContent: function($productForm, $notifyForm, productData) {
    ...
    //Metafield_swatches globally defined at collection-template.liquid line 8
    metafield_swatch = metafield_swatches.find((it) => it.product_id == productData.product_id);
    if(metafield_swatch) metafield_swatch.init();
  ...
  if (globalQuickShopProduct != 'undefined'){
    ...
    $('.js-savings').html('');
      ...
      //Title and Vendor
      $('.js-product-title').html('<a href="'+ product.product_in_collection_url +'" title="'+ product.escaped_title +'">'+ product.title.split("|")[0] +'</a>');
	  $('.js-product-vendor')
